//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.0

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

part of openapi.api;

class OrderModelCommentCreate {
  /// Returns a new [OrderModelCommentCreate] instance.
  OrderModelCommentCreate({
    @required this.comment,
  });

  String comment;

  @override
  bool operator ==(Object other) => identical(this, other) || other is OrderModelCommentCreate &&
     other.comment == comment;

  @override
  int get hashCode =>
  // ignore: unnecessary_parenthesis
    (comment == null ? 0 : comment.hashCode);

  @override
  String toString() => 'OrderModelCommentCreate[comment=$comment]';

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
      json[r'comment'] = comment;
    return json;
  }

  /// Returns a new [OrderModelCommentCreate] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static OrderModelCommentCreate fromJson(dynamic value) {
    if (value is Map) {
      final json = value.cast<String, dynamic>();
      return OrderModelCommentCreate(
        comment: mapValueOfType<String>(json, r'comment'),
      );
    }
    return null;
  }

  static List<OrderModelCommentCreate> listFromJson(dynamic json, {bool emptyIsNull, bool growable,}) =>
    json is List && json.isNotEmpty
      ? json.map(OrderModelCommentCreate.fromJson).toList(growable: true == growable)
      : true == emptyIsNull ? null : <OrderModelCommentCreate>[];

  static Map<String, OrderModelCommentCreate> mapFromJson(dynamic json) {
    final map = <String, OrderModelCommentCreate>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) => map[key] = OrderModelCommentCreate.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of OrderModelCommentCreate-objects as value to a dart map
  static Map<String, List<OrderModelCommentCreate>> mapListFromJson(dynamic json, {bool emptyIsNull, bool growable,}) {
    final map = <String, List<OrderModelCommentCreate>>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) {
          map[key] = OrderModelCommentCreate.listFromJson(
            value,
            emptyIsNull: emptyIsNull,
            growable: growable,
          );
        });
    }
    return map;
  }
}

