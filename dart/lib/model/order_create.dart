...
class OrderCreate {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "OrderCreate[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "OrderCreate[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, List<OrderCreate>> mapListFromJson(
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = OrderCreate.listFromJson(
          entry.value,
          growable: growable,
        );
      }
