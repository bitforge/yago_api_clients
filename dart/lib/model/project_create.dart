...
class ProjectCreate {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ProjectCreate[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ProjectCreate[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ProjectCreate> listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ProjectCreate.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ProjectCreate> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = ProjectCreate.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<ProjectCreate>> mapListFromJson(
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = ProjectCreate.listFromJson(
          entry.value,
          growable: growable,
        );
      }
