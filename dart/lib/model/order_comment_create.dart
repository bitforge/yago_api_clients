...
class OrderCommentCreate {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "OrderCommentCreate[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "OrderCommentCreate[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<OrderCommentCreate> listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = OrderCommentCreate.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, OrderCommentCreate> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = OrderCommentCreate.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<OrderCommentCreate>> mapListFromJson(
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = OrderCommentCreate.listFromJson(
          entry.value,
          growable: growable,
        );
      }
