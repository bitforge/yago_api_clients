...
class OrderModelUpdate {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "OrderModelUpdate[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "OrderModelUpdate[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<OrderModelUpdate> listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = OrderModelUpdate.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, OrderModelUpdate> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = OrderModelUpdate.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<OrderModelUpdate>> mapListFromJson(
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = OrderModelUpdate.listFromJson(
          entry.value,
          growable: growable,
        );
      }
